...
From this graph, we can see that Daphnie captures the most compared to the other characters throughout all of the Scooby-Doo series. However, it is fairly close amongst the characters, as no one captures dramatically less than any of the other characters.
...
This graph allows up to see the variability among the capturing for each character for each series. We can see that each character has a fair amount of low numbers of captures for the season. Additionally, each of the characters has a high capture amount for Warner Home Video, either with it being the highest or second highest count for the character.
...
From this visualization we can see that when there is a whole truckload of snacks eaten, Velma says her catchphrase more often, though not by much.
...
This graph shows us that Scooby catches a higher amount more consistantly than the other characters, and that it is not just one show where he has a high caught count that was bringing up the overall. We can see that it is Fred who actually has the highest caught count in one series: Scooby-Doo Mystery Incorporated. Additionally, we can see that in Shaggy & Scooby-Doo Get a Clue! there is a high amount of times that the culprit was not caught.
...
This visualization shows us that when the monster is real, it is a lot more likely to not be caught. Although, Scooby is still the most likely to catch it out of al of the characters.
...
Finally, we see that when there is a trap set up before catching the monster, Fred is now much more likely to be the one to catch the monster.
