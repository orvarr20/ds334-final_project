...
server <- function(input, output, session){
  ...
  observeEvent(input$filtering_var, {
    if(input$filtering_var == "none"){
      ...
      filtering_var_vec <-df |> 
       filter(series_name == input$series_select) |> ##why does it still have all the levels??
       pull(input$filtering_var) |>
       levels()
      updateSelectInput(inputId = "sub_filter_var", choices = filtering_var_vec)
    }
  })
      ...
      if(input$var_select == "Catchphrase"){
        ...
        if(input$var_select == "Caught"){
          caught_df |> filter(series_name == input$series_select) |>
            filter(monster_type == input$sub_filter_var) |>
            group_by(who) |> summarise(count = sum(caught_by)) |>
            ggplot(aes(x = who, y = count)) +
            geom_col(position = "dodge", fill = c("#7c68ae", "#76a2ca", "black", "red",
                                                  "#B87333","#b2bb1b", "#F8991D")) +
...
#need to figure out why some of them are blank, no background whereas some are background but no bars
#also need to figure out why I can have filter(input$filtering_var == __)
#then need to figure out what to do about "none" because I still want the default graph I had before... probably need an if_else :(
#then add it to all other graphs and then all the tables
```

homework: 
- make more plots for the write up (include variability between the episodes somehow -> maybe make them reactive so you can see what episode is what and color by season)
-make filtering work for caught graph
