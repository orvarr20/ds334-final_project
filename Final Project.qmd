    ...
    if(input$filtering_var == "none"){
      ...
      if(input$var_select == "Captured"){
        if(input$series_select == "none"){
          filtering_vec <- captured_df |> select(input$filtering_var) |>
                                       distinct()
    ...
    if(input$var_select == "Captured"){
      if(input$series_select == "none"){
        captured_df |> group_by(who) |> summarise(count = sum(captured_by)) |>
          filter(input$filtering_var == input$sub_filter_var) |>
          ggplot(aes(x = who, y = count)) +
          geom_col(position = "dodge", fill = c("#7c68ae", "#76a2ca",
                                                "#B87333","#b2bb1b", "#F8991D")) +
...
Homework:
- get jitter plots to color by character
-get filtering working for the overall graphs and tables (ISSUE IS WITH INPUT$FILTERING_VAR) --> works with monster_type == input$sub_filter_var but not with input$filtering_var == "Mechanical" --> need to change something in ObserveEvent??
- add filtering to all other variables, graphs, and tables

```{r}
captured_df |> filter(series_name == "A Pup Named Scooby-Doo") |>
            filter(monster_type == "Mechanical") |>
          group_by(who) |> summarise(count = sum(captured_by)) |>
          ggplot(aes(x = who, y = count)) +
          geom_col(position = "dodge", fill = c("#7c68ae", "#76a2ca",
                                                "#B87333","#b2bb1b", "#F8991D")) +
