  ...
  output$barplot <- renderPlot({
    if(input$var_select == "Captured"){
      if(input$series_select == "none"){
        ...
        ##ISSUE HERE: In argument .data[["monster_type"]] = input$sub_filter_var
        ## column monster_type not found in .data
        else{
          captured_df |> group_by(who) |> summarise(count = sum(captured_by)) |>
...
Homework:
-finish making table work for captured_df with and without series_select using smaller app below
-update observe event so that if ANY of them change (series_select, filtering_var, or var_select changes it updates the options for sub_filtering_var)
-make filtering work for overall datasets too

Monday class:
- add filtering to all other variables, graphs, and tables
(need to use .data[[input$filtering_var]]!!)
  ...
  output$barplot <- renderPlot({
    if(input$var_select == "Captured"){
      if(input$series_select == "none"){
        ...
        ##ISSUE HERE (I think): In argument .data[["monster_type"]]=input$sub_filter_var
        ## column monster_type not found in .data
        else{
