  ...
  observeEvent(input$filtering_var, {
    if(input$filtering_var == "none"){
      ...
      if(input$var_select == "Catchphrase"){
        if(input$series_select == "none"){
          filtering_vec <- catchphrase_df |> select(input$filtering_var) |>
            distinct()
          updateSelectInput(inputId = "sub_filter_var", choices = filtering_vec)
        }
        else{
...
Wednesday class:
-what's up with sometimes updating sometimes not?? in observe Event
- get filtering to work for overall graphs and tables. The issue is with input$filtering var.
(it works with monster_type == input$sub_filter_var but with input$filtering_var == "Mechanical)
-maybe need to change something in ObserveEvent??

Homework:
- add filtering to all other variables, graphs, and tables

Monday class:
- do the entire write-up

Homework:
-make updated blog post

Wednesday class:
-relook over everything, make sure everything is working, submit
