...
server <- function(input, output, session){
  ...
  output$barplot <- renderPlot({
    if(input$var_select == "Catchphrase"){
      ...
      geom_col(position = "dodge", fill = c("#7c68ae", "#F8991D", "#B87333", "#b2bb1b")) +
      theme_minimal() +
      labs(title = glue::glue("Catchphrase count for ", input$series_select))
    }
    if(input$var_select == "Snacks"){
      snacks_df |> filter(series_name == input$series_select) |>
        ...
        geom_col(position = "dodge", fill = c("#7c68ae", "#76a2ca",
                                              "#B87333","#b2bb1b", "#F8991D")) +
        labs(title = "Snack count by character for ", input$series_select)
    }
  })
...
Next class session:
-for the remaining variables, make sure that all 5 people are shown even if they have 0 like in snacks
-add the remaining variables to the shiny app(captured, caught, and unmasked)
- make x and y axis titles better for each graph and theme minimal
