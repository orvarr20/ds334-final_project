...
server <- function(input, output, session){
  ...
  observeEvent(input$filtering_var, {
    if(input$filtering_var == "none"){
      ...
      filtering_var_vec <- df |> select(input$filtering_var) |> distinct()
...
next class:
-finish making the levels of all the monster variables nicer
-start filtering by monster attributes

```{r}
df |> group_by(monster_amount) |> summarise(count = n())
##fine, just need to put in numerical order in the app
