...
captured_plot <- captured_df|> group_by(who, series_name) |> summarise(count = sum(captured_by)) |>
  ...
  geom_jitter(width = 0.1) ##how to get this to be filled by who??
ggplotly(captured_plot, tooltip = "label")
...
catchphrase_plot <- catchphrase_df|> group_by(catchphrase, series_name) |> summarise(count = sum(Count)) |>
  ggplot(aes(x = catchphrase, y = count, label = series_name)) +
  geom_jitter(width = 0.1) 
ggplotly(catchphrase_plot, tooltip = "label")
...
caught_plot <- caught_df|> group_by(who, series_name) |> summarise(count = sum(caught_by)) |>
  ggplot(aes(x = who, y = count, label = series_name)) +
  geom_jitter(width = 0.1) ##how to get this to be filled by who??
ggplotly(caught_plot, tooltip = "label")
...
snacks_plot <- snacks_df|> group_by(who, series_name) |> summarise(count = sum(snacks)) |>
  ggplot(aes(x = who, y = count, label = series_name)) +
  geom_jitter(width = 0.1) ##how to get this to be filled by who??
ggplotly(snacks_plot, tooltip = "label")
...
unmasked_plot <- unmasked_df|> group_by(who, series_name) |> summarise(count = sum(unmasked)) |>
  ggplot(aes(x = who, y = count, label = series_name)) +
  geom_jitter(width = 0.1) ##how to get this to be filled by who??
ggplotly(unmasked_plot, tooltip = c("label"))
```
...
server <- function(input, output, session){
  ...
  observeEvent(input$filtering_var, {
    if(input$filtering_var == "none"){
      ...
      if(input$var_select == "Captured"){
        filtering_vec <- captured_df |>
                                     filter(series_name == input$series_select) |>
                                     distinct(input$filtering_var) |>
                                     pull()
        updateSelectInput(inputId = "sub_filter_var", choices = filtering_vec)
      }
      ##get this to work and then do it for all other vars
    }
  })
  ...
  output$table <- renderDataTable({
    if(input$var_select == "Captured"){
      ...
      if(input$var_select == "Catchphrase"){
        if(input$series_select == "none"){
          catchphrase_df |>mutate(Catchphrase = catchphrase) |> group_by(Catchphrase) |>
...
next class:
-questions: how to get filtering to work!!! And for jitter plot is it possible to color by person?? and then add to every graph
- get filtering work for captured graph (the drop downs and graph)
- add filtering to all other variables
- add filtering to tables

```{r}
#mechical (bakcground) vs. magician (empty)
##magician is empty because there are none in that series --> gonna be an issue for the table... maybe need to change the vector to accommodate available options specific to that season...
#mechanical IS in the series but no one caught it?? but not in caught_not

caught_df |> filter(series_name == "Scooby Doo, Where Are You!") |>
  group_by(monster_type) |> summarise(n())

df |> filter(series_name == "Scooby Doo, Where Are You!") |> group_by(monster_type) |> summarise() #just doing this and changing to input doesn't work!!

df |> filter(series_name == "Scooby Doo, Where Are You!") |> select(monster_type)


df |> filter(series_name == "Scooby Doo, Where Are You!") |> group_by(monster_type) |>
  pull(monster_type) |> levels()

df |> filter(series_name == "Scooby Doo, Where Are You!") |> distinct(monster_type) |> 
  pull(monster_type)

captured_df |> filter(series_name == "Scooby Doo, Where Are You!") |> distinct(monster_type) |> pull(monster_type)
...
captured_df |> filter(series_name == "A Pup Named Scooby-Doo") |>
